...
tabCol = np.zeros((N,N,3), dtype=float)
...
def fillArr():
    for i, u in enumerate(u_vals):
        for j, v in enumerate(v_vals):
            ...
            tabCol[i,j]=[colorR,colorG,colorB]
        for i in range(N):
            tabCol[i, N-1] = tabCol[i, 0]
...
def render(time,angle):
    ...
    for i in range(N - 1):
        for j in range(N):
            glColor3fv(tabCol[i,j])
            glVertex3fv(tab[i, j])
            glColor3fv(tabCol[i+1,j])
            glVertex3fv(tab[i + 1, j])
        glColor3fv(tabCol[i,N-1])
        glVertex3fv(tab[i, N-1])

        glColor3fv(tabCol[i,N-1])
        glVertex3fv(tab[i + 1, N-1])

        glColor3fv(tabCol[i,0])
        glVertex3fv(tab[i, 0])

        glColor3fv(tabCol[i+1,0])
...
def update_viewport(window, width, height):
    ...
    if width <= height:
        glOrtho(-7.5, 7.5, -7.5 / aspect_ratio, 7.5 / aspect_ratio, 0.1, 100.0)
    else:
