...
Po rozpoczęciu zajęć i zapoznaniu się z zakresem działań do wykonania przystąpiono do identyfikacji drukarki. Po wystarczających wyszukiwaniach zidentyfikowano drukarkę jako HP LaserJet 5L, która podłączona została złączem równoległym DB-25 do komputera. Zapoznano się z instrukcją obsługi drukarki.
#figure(image("drukarka.jpg", width:70%),caption: [Zdjęcie drukarki HP LaserJet 5L],supplement: [Zdjęcie])
#pagebreak()
Rozpoczęto od próby prostego wydruku pojedyńczego słowa  wykorzystując wiersz poleceń. Po poprawnym wydruku rozszerzono działanie o wydruk pliku z rozszerzeniem .txt, kopiując jego zawartość do *LPT3*, co przenosiło informacje bezpośrednio do drukarki.
...
Następnie rozpoczęto dodawanie usprawnień do programu. W wyniku końcowym udało się modyfikować czcionkę oraz modyfikować cały tekst. Nie udało się wprowadzić modyfikację fragmentów tekstu. Pomimo próby nie udało się zaimplementować druku grafiki. Udało się za to wprowadzić selekcję sposobu wydruku pomiędzy portait i landscape.
...
= Podsumowanie
W wyniku wykonanego zadania zapoznano się ze sposobem działania drukarek, sposobem komunikacji z drukarką poprzez port równoległy oraz zapoznano się z poleceniami PCL. Pomimo nie wykonania zadania w stu procentach (brak druku grafiki) wykonano jego dużą część, poprawnie drukując zmodyfikowany tekst oraz modyfikując orientację tekstu.
...
#figure(image("1.jpg", width:100%),caption: [Kartka zawierająca testowe wydruki],supplement: [Zdjęcie])
#figure(image("2.jpg", width:100%),caption: [Kartka zawierająca testowe wydruki],supplement: [Zdjęcie])
